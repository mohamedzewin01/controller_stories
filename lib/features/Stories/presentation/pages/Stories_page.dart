...
Widget _buildStatisticsCard(List<Stories> stories) {
  ...
  int maleStories = stories.where((story) => story.gender == 'male').length;
  int femaleStories = stories.where((story) => story.gender == 'female').length;
  ...
  return Container(
    ...
    child: Row(
      children: [
        ...
        Expanded(
          child: _buildStatItem('للأولاد', maleStories.toString()),
        ),
        Container(
          width: 1,
          height: 40,
        ...
        Expanded(
          child: _buildStatItem('للبنات', femaleStories.toString()),
        ),
      ],
    ),
  );
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              _buildTag(
                                story.gender == 'male' ? 'ولد' : 'بنت',
                                story.gender == 'male' ? Colors.blue : Colors.pink,
                              ),
                    ...
                    PopupMenuButton<String>(
                      ...
                      itemBuilder: (context) => [
                        PopupMenuItem(
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.info_rounded, size: 18, color: Colors.blue[600]),
                              const SizedBox(width: 8),
                              const Text('التفاصيل'),
                            ],
                          ),
                        ...
                        PopupMenuItem(
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.edit_rounded, size: 18, color: Colors.orange[600]),
                              const SizedBox(width: 8),
                              const Text('تعديل'),
                            ],
                          ),
...
void _showAddStoryDialog() {
  showModalBottomSheet(
    ...
    builder: (context) => StoryFormDialog(
      ...
      onSave: (title, description, problemId, gender, ageGroup, imageCover) async {
...
class StoryFormDialog extends StatefulWidget {
  final Stories? story;
  final int categoryId;
  final Function(String title, String description, int problemId, String gender, String ageGroup, File imageCover) onSave;
...
class _StoryFormDialogState extends State<StoryFormDialog> {
  ...
  final List<String> _ageGroupOptions = ['3-5', '6-8', '9-12'];
  ...
  void initState() {
    ...
    if (widget.story != null) {
      ...
      _selectedGender = widget.story!.gender ?? 'male';
      _selectedAgeGroup = widget.story!.ageGroup ?? '3-5';
    }
  }
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Icon(
                        widget.story == null ? Icons.add_rounded : Icons
                            .edit_rounded,
                        color: ColorManager.primaryColor,
                      ),
                    ),
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        await widget.onSave(
          ...
          _selectedAgeGroup,
          _selectedImage ??
              File(''), // For edit mode, API should handle null image
        );
      } finally {
        if (mounted) {
